...
\begin{definicija}
    Če za element \(x \in X\) obstaja nosilec \(A\), ki je končna množica, potem je \(x\) \textbf{končno podprt} (angl. \emph{finitely supported}).
\end{definicija}
...
Naj bodo \(\Gamma,\ x \vdash M,\ M'\), \(\Gamma \vdash N\), \(M \ \#\ N \ \vert\ \Gamma\) in \(M' \ \#\ N \ \vert\ \Gamma\). Definirajmo funkcijo \(\sub_{x,\ N}\): \[\sub_{x,\ N}(M) := M[N/x].\]
...
% Treba je tu dodati še tranzitivno-refleksivno ovojnico, da je izrek pravilen.
