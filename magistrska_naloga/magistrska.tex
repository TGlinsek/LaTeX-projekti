...
\subsection{Alfa ekvivalenca}

Zanimalo nas bo, kdaj sta dva izraza ekvivalentna v nekem smislu: želimo, uvesti ekvivalenco izrazov, ki pove, kdaj sta izraza enaka po pomenu. Natančneje, želimo, da sta dva izraza ekvivalentna, če lahko enega dobimo iz drugega le s preimenovanjem vezanih spremenljivk. Tej ekvivalenci pravimo alfa ekvivalenca.
...
\begin{definicija}
    ...
    \]
    \[
        \inference{\Gamma \vdash M \quad \Gamma \vdash N \quad M \sim N | \Gamma}{\Gamma \vdash M \equiv_\alpha N}
    \]
    Če za dva izraza velja \(\Gamma \vdash M \equiv_\alpha N\), potem rečemo, da sta izraza \emph{alfa-ekvivalentna}.
\end{definicija}
Opomba: \(\equiv_\alpha\) je kontekstualizirana kongruenčna relacija.

Opomba: iz produkcijskih pravil je razvidno, da sta dva izraza lahko alfa-ekvivalentna le, če ju opazujemo nad istim kontekstom.
...
\begin{trditev}
    Naj \(\Gamma \vdash M\) izraz nad kontekstom \(\Gamma\) in naj bo še \(N\) en drug izraz, tako da velja \(M \sim N | \Gamma\). Potem velja \(\Gamma \vdash N\), tj. izraz \(N\) živi v kontekstu \(\Gamma\).
\end{trditev}

\noindent
{\em Dokaz:\/}
Pogoj \(M \sim N | \Gamma\) implicira obstoj permutacije \(\pi\), ki fiksira člene zaporedja \(\Gamma\) in da velja \(\pi(M) = N\). Sledi \(\pi(\Gamma \vdash M) = \pi(\Gamma) \vdash \pi(M) = \Gamma \vdash N\).
\qed
...
\begin{lema}
    Naj \(A, B\ \text{in}\ C\) nominalne množice, \(a, b, c\) pa elementi vsak svoje množice izmed teh. Potem obstaja \(b'\), tako da \(b' \sim b | c\) in \(b' \# a | c\).
\end{lema}
\noindent
{\em Dokaz:\/} TODO - ne razumem še formulacije tega, niti kako se bo ta lema kasneje uporabljala. \qed
...
\begin{definicija}
    ...
    \[
        \inference{}{\Gamma, x, \Gamma' \vdash x \rightarrow_\beta x}
    \]
    \[
    ...
    \]
    \[
        \inference{\Gamma, x \vdash M \quad \Gamma \vdash N \quad N' \sim N | \Gamma \quad M \# N' | \Gamma}{\Gamma \vdash (\lambda x.M)N \rightarrow_\beta M[N'/x]}
    \]
\end{definicija}
...
\begin{lema}
    ...
    \begin{enumerate}
        \item \(M \sim N | \Gamma\), oz.
        \item \(\Gamma \vdash M \equiv_\alpha N\)
    \end{enumerate}
    Potem velja, da
    \begin{enumerate}
        \item obstaja \(N'\), da \(M' \sim N' | \Gamma\) in \(\Gamma \vdash N \rightarrow_\beta N'\), oz.
        \item obstaja \(N'\), da \(\Gamma \vdash M' \equiv_\alpha N' \) in \(\Gamma \vdash N \rightarrow_\beta N'\)
    \end{enumerate}
\end{lema}
To je torej v bistvu več lem (štiri so) naenkrat. Bomo videli, katere je lažje najprej dokazati oz. katere sploh držijo.
